...
const SITE_URL = typeof window !== "undefined" ? window.location.origin : "https://feedbackhub.com";
...
export const SEO = ({
  ...
  useEffect(() => {
    const defaultTitle = t("seo.defaultTitle", { defaultValue: "FeedbackHub - Платформа анонимной обратной связи" });
    const defaultDescription = t("seo.defaultDescription", {
      defaultValue: "Отправляйте анонимные отзывы в свою компанию. Безопасная платформа для честной обратной связи.",
    });

    const pageTitle = title ? `${title} | FeedbackHub` : defaultTitle;
    ...
    updateMetaTag("author", "FeedbackHub");
    ...
    updateMetaTag("og:site_name", "FeedbackHub", true);
    ...
    languages.forEach((lang) => {
      ...
      hreflang.setAttribute("href", `${SITE_URL}${location.pathname}?lang=${lang}`);
    });
    ...
    xDefault.setAttribute("href", `${SITE_URL}${location.pathname}`);

    // Update HTML lang attribute
    document.documentElement.setAttribute("lang", currentLang);
  }, [title, description, keywords, image, type, noindex, canonical, location.pathname, t, i18n.language]);
    ...
    <StructuredData
      ...
      data={{
        ...
        potentialAction: {
          ...
          target: {
            "@type": "EntryPoint",
            urlTemplate: `${SITE_URL}/check-status?messageId={search_term_string}`,
          },
