...
const Welcome = () => {
  ...
  const { data: company, isLoading: isValidating } = useQuery({
    ...
    queryFn: () => companyApi.getByCode(companyCode),
  ...
  const handleValidateCode = async () => {
    ...
    const foundCompany = await companyApi.getByCode(companyCode);
    if (!foundCompany) {
  ...
  const handleSendMessage = () => {
    ...
    navigate(`/send-message?code=${validatedCode}`);
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {isAuthenticated ? (
                <Button variant="ghost" size="sm" onClick={() => navigate("/company")} className="text-xs sm:text-sm">
                  <span className="hidden sm:inline">{t("company.dashboard")}</span>
                  <span className="sm:hidden">{t("company.dashboard")}</span>
                </Button>
              ) : (
                <div className="flex items-center gap-1 sm:gap-2">
                  <Button variant="ghost" size="sm" onClick={() => navigate("/register")} className="text-xs sm:text-sm px-2 sm:px-3">
