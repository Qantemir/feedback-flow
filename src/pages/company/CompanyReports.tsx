...
import { statsApi, companyApi } from "@/services/api";
...
const CompanyReports = () => {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    queryKey: ["stats", user?.companyId],
    queryFn: () => statsApi.getCompanyStats(user?.companyId || 0),
    enabled: !!user?.companyId,
  });

  const { data: growthMetrics, isLoading: growthLoading } = useQuery({
    queryKey: ["growth", user?.companyId],
    queryFn: () => statsApi.getGrowthMetrics(user?.companyId || 0),
    enabled: !!user?.companyId,
  });
